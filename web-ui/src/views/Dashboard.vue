<template>
  <section class="space-y-6">
    <div class="rounded-[26px] border border-stone-200/60 bg-[rgba(255,253,248,0.56)] p-5 shadow-[0_8px_24px_rgba(71,52,31,0.035)] lg:p-6">
      <div class="text-[11px] font-semibold uppercase tracking-[0.2em] text-stone-400">{{ t('dashboardOverview') }}</div>
      <h2 class="mt-2 text-[2rem] font-semibold tracking-tight text-stone-900">{{ t('dashboardSnapshot') }}</h2>
      <p class="mt-3 max-w-2xl text-sm leading-7 text-stone-600">
        {{ t('dashboardSnapshotDescription') }}
      </p>
    </div>

    <div v-if="loading" class="memory-panel rounded-[24px] p-6 text-sm text-stone-500">{{ t('dashboardLoading') }}</div>
    ...
    <template v-else>
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="memory-panel rounded-[24px] p-5">
          <div class="text-[11px] uppercase tracking-[0.18em] text-stone-400">{{ t('dashboardTotalConversations') }}</div>
          <div class="mt-3 text-3xl font-semibold text-stone-900">{{ totalConversations }}</div>
        </div>
        <div class="memory-panel rounded-[24px] p-5">
          <div class="text-[11px] uppercase tracking-[0.18em] text-stone-400">{{ t('dashboardPlatforms') }}</div>
          <div class="mt-3 text-3xl font-semibold text-stone-900">{{ platformCount }}</div>
        </div>
        <div class="memory-panel rounded-[24px] p-5">
          <div class="text-[11px] uppercase tracking-[0.18em] text-stone-400">{{ t('dashboardVectorDocs') }}</div>
          <div class="mt-3 text-3xl font-semibold text-stone-900">{{ vectorDocs }}</div>
        </div>
        <div class="memory-panel rounded-[24px] p-5">
          <div class="text-[11px] uppercase tracking-[0.18em] text-stone-400">{{ t('dashboardAvgImportance') }}</div>
...
const avgImportance = computed(() => {
  ...
  const totalCount = platforms.reduce((acc, p) => acc + (p.count || 0), 0)
  if (totalCount === 0) return '0.0'
  const weightedSum = platforms.reduce((acc, p) => acc + (p.avg_importance || 0) * (p.count || 0), 0)
...
onMounted(async () => {
  try {
    ...
    error.value = t('dashboardLoadFailed', { message: e.message })
  } finally {
    loading.value = false
  }
})
