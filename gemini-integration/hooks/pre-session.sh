...
# Gemini CLI Pre-Session Hook
# Injects context from Memory Hub into GEMINI.md before Gemini CLI starts.

WORKING_DIR="${GEMINI_WORKING_DIR:-$(pwd)}"
...
# Execute switch to inject context into GEMINI.md
RESULT=$(curl -s -X POST "$MEMORY_HUB_URL/api/v2/switch" \
  -H "Content-Type: application/json" \
  -d "{\"to_cli\": \"gemini_cli\", \"workspace_path\": \"$WORKING_DIR\"}")
...
if [ "$STATUS" = "ok" ]; then
    ...
    echo "Context injected to GEMINI.md ($TOKENS tokens)"
else
